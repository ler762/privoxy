...
long flush_iob(jb_socket fd, struct iob *iob, unsigned int delay)
{
   ...
   log_error(LOG_LEVEL_IO, "flush_iob %d len=%ld", fd, len);
   ...
   if ( (status = write_socket_delayed(fd, iob->cur, (size_t)len, delay)) )
   {
      log_error(LOG_LEVEL_IO,
                "flush_iob %d write error: %d", fd, status);
      return(-1);
   }
...
jb_err add_to_iob(struct iob *iob, const size_t buffer_limit, char *src, long n)
{
   ...
   if (need > iob->size)
   {
      ...
      log_error(LOG_LEVEL_IO,
          "add_to_iob: reallocate iob->buf old: %d  need: %d  want: %d  limit: %d",
          iob->size, need, want, buffer_limit);
   ...
   log_error(LOG_LEVEL_IO, "add_to_iob: memcpy %d bytes", (size_t)n);
...
jb_err decompress_iob(struct client_state *csp)
{
   ...
   log_error(LOG_LEVEL_CONNECT, "decompress_iob: begin");
   ...
   while (Z_BUF_ERROR == (status = inflate(&zstr, Z_FINISH)))
   {
      ...
      log_error(LOG_LEVEL_CONNECT, "decompress_iob: realloc %d", bufsize);
   ...
   log_error(LOG_LEVEL_CONNECT, "decompress_iob: decompressed");
   ...
   log_error(LOG_LEVEL_CONNECT, "decompress_iob: finished");
   return JB_ERR_OK;
...
static jb_err server_save_content_length(struct client_state *csp, char **header)
{
   ...
   else
   {
      ...
      log_error(LOG_LEVEL_HEADER, "csp->expected_content_length=%llu", csp->expected_content_length);
      csp->flags |= CSP_FLAG_SERVER_CONTENT_LENGTH_SET;
...
jb_err get_destination_from_headers(const struct list *headers, struct http_request *http)
{
   ...
   log_error(LOG_LEVEL_HEADER, "Faked request-Line: %s", http->cmd);
