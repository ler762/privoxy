   ...
   for ( j = 0; j < numIosizeCounters; j++ ) {
      if ( myCounters[j] > 0 ) {
         ...
         snprintf(buf, sizeof(buf), "%s: %10u   %6.2f  %10u\n",
                  iosizeCounterDesc[j], myCounters[j], percent, iosizeRunLenMax[j]);
...
/*********************************************************************
 *
 * Function    :  cgi_show_url_info_x
 *
 * Description :  CGI function (mainly for programatic use) that shows
 *                which actions Privoxy will perform for a given url
 *
 * Parameters  :
 *          1  :  csp = Current client state (buffers, headers, etc...)
 *          2  :  rsp = http_response data structure for output
 ...
 *
 *********************************************************************/
jb_err cgi_show_url_info_x(struct client_state *csp,
                           struct http_response *rsp,
                           const struct map *parameters)
{
   char *url_param;
...
"<style><!-- td.box { border: solid black 1px; background-color: #eeeeee; } --></style>\n"\
"</head> <body>\n"\
...
"<p>Requests for <a href=\"@url@\">@url@</a> will be\n"\
"<!-- @if-no-forwarder-start -->\n"\
"made <b>directly</b>, no forwarding settings apply\n"\
"<!-- if-no-forwarder-end@-->\n"\
"<!-- @if-http-forwarder-start -->\n"\
"<!-- @if-socks-forwarder-start -->first <!-- if-socks-forwarder-end@ -->\n"\
...
"<!-- @if-http-forwarder-start -->\n"\
"<!-- @if-socks-forwarder-start -->and then <!-- if-socks-forwarder-end@ -->\n"\
"<!-- if-http-forwarder-end@ -->\n"\
"<!-- @if-http-forwarder-start -->\n"\
...
"</td></tr>\n"\
"<!-- @if-valid-url-start -->\n"\
"<tr><td class=\"box\"><h2>Final results:</h2><b>@final@</b></td></tr>\n"\
"<!-- if-valid-url-end@ -->\n"\
"<!-- if-url-given-end@ -->\n"\
"<tr><td class=\"box\">\n"\
"<h2>Look up the actions for a URL:</h2>\n"\
"<form method=\"GET\" action=\"@default-cgi@show-url-info-x\"><p>\n"\
"<input type=\"text\" name=\"url\" size=\"80\" value=\"@url@\" class=\"url\">\n"\
"<input type=\"submit\" value=\"Go\"></p></form></td></tr></table>\n"\
"</body>\n"\
"</html>\n" ;
#endif  /* gofaster */
    ...
    *  sure the config files are current
    *
    *  XXX: is there a cgi call for that? :XXX
    */

   if (NULL == (exports = default_exports(csp, "show-url-info-x")))
   {
      return JB_ERR_MEMORY;
   }
   ...
   else
   {
      ...
      else if (err)
      {
         ...
         return template_fill_for_cgi(csp, "show-url-info-x", exports, rsp);
      }
       ...
       *
       * From here on the action bitmask will no longer reflect
       * the real url (http://config.privoxy.org/show-url-info-x?url=.*),
       * but luckily it's no longer required later on anyway.
       */
      free_current_action(csp->action);
      get_url_actions(csp, url_to_query);
