...
BEGIN {
  prev = "@";  prevlen = 1;
  stderr = "/dev/stderr"
  mylocalhost = "localhost.home.net"
  webserver = "/inet/tcp/0/localhost/8118"
...

{
  ...
  if ( ( url ~ /^localhost\.?$/ ) || ( url == mylocalhost ) ) {
       # localhost  localhost.  localhost.${MYDOMAIN}
       # what other names are there for "this machine"???
  ...
  else if ( blocked != "-" ) printf("############################# unknown:%s: %s\n", blocked, url)
...
# print "GET http://config.privoxy.org/show-url-info-x?url=" url " HTTP/1.1"  |& webserver
